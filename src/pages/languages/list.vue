<template>
  <div>
    <Breadcrumbs :title="$t('language.heading')" />
...
export default {
  ...
  methods: {
    ...
    deleteLanguage(item) {
      this.confirmModal(this.$t("language.deleteConfirm")).then((result) => {
        if (result.value) {
          this.setLoadingState(true);
          languageService.deleteLanguage(item.item).then(
            () => {
              this.items = [...this.items.filter((x) => x.id != item.item.id)];
              this.setLoadingState(false);
              this.serverRestartRequired(true);
              this.successModal(this.$t("language.deleteSuccess"));
            },
            (error) => {
              console.log("httperror  => ", error);
              this.setLoadingState(false);
    ...
    editLanguage(item) {
      this.setLanguage(item.item);
      this.$router.push("/language/update/" + item.item.id);
    },
  },
};
</script>