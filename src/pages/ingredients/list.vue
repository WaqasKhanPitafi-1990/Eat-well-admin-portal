<template>
  <div>
    <Breadcrumbs :title="$t('ingredient.heading')" />
...
export default {
  ...
  methods: {
    ...
    deleteIngredient(item) {
      this.confirmModal(this.$t("ingredient.deleteConfirm")).then((result) => {
        if (result.value) {
          this.setLoadingState(true);
          ingredientService.deleteIngredient(item.item).then(
            () => {
              this.items = [...this.items.filter((x) => x.id != item.item.id)];
              this.setLoadingState(false);
              this.serverRestartRequired(true);
              this.successModal(this.$t("ingredient.deleteSuccess"));
            },
            (error) => {
              console.log("httperror  => ", error);
              this.setLoadingState(false);
    ...
    editIngredient(item) {
      this.setIngredient(item.item);
      this.$router.push("/ingredients/update/" + item.item.id);
    },
  },
};
</script>