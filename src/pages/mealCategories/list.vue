<template>
  <div>
    <Breadcrumbs :title="$t('mealCategory.heading')" />
                  ...
                  <template #cell()="data">
                    <template v-if="data.field.key == 'action'">
                      <div class="action-buttons">
                        <feather
                          @click="deleteMealCategory(data)"
                          class="pointer"
                          type="trash"
                        ></feather>
                        <feather
                          @click="editMealCategory(data)"
                          class="pointer"
                          type="edit"
                        ></feather>
                      </div>
...
export default {
  ...
  methods: {
    ...mapActions("user", ["setMealCategory"]),
    ...
    deleteMealCategory(item) {
      this.confirmModal(this.$t("mealCategory.deleteConfirm")).then(
        (result) => {
          if (result.value) {
            this.setLoadingState(true);
            mealCategoryService.deleteMealCategory(item.item).then(
              () => {
                this.items = [
                ...
                this.successModal(this.$t("mealCategory.deleteSuccess"));
              },
              (error) => {
                console.log("httperror  => ", error);
                this.setLoadingState(false);
    ...
    editMealCategory(item) {
      this.setMealCategory(item.item);
      this.$router.push("/meal-categories/update/" + item.item.id);
    },
  },
};
</script>