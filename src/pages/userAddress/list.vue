<template>
  <div>
    <Breadcrumbs :title="$t('userAddress.heading')" />
                  ...
                  <template #cell()="data">
                    <template v-if="data.field.key == 'action'">
                      <div class="action-buttons">
                        <feather
                          @click="deleteUserAddress(data)"
                          class="pointer"
                          type="trash"
                        ></feather>
                        <feather
                          @click="editUserAddress(data)"
                          class="pointer"
                          type="edit"
                        ></feather>
                      </div>
                    ...
                    <template
                      v-else-if="
                        data.field.key.toLowerCase().includes('default')
                      "
                    >
                      <feather
                        :class="{
                    ...
                    <template
                      v-else-if="
                        data.field.key.toLowerCase().includes('addresstype') &&
                        data.value
                      "
                    >
...
export default {
  ...
  methods: {
    ...mapActions("user", ["setUserAddress"]),
    ...
    fetchData() {
      ...
      userAddressService.getAll().then(
        (response) => {
          ...
          if (this.items && this.items.length)
            Object.keys(this.items[0]).forEach((key) => {
              if (!key.toLowerCase().includes("addresstype")) {
    ...
    deleteUserAddress(item) {
      this.confirmModal(this.$t("userAddress.deleteConfirm")).then((result) => {
        if (result.value) {
          this.setLoadingState(true);
          userAddressService.deleteUserAddress(item.item).then(
            (response) => {
              this.items = response.data.data;
              this.totalRows = this.items.length;
              this.setLoadingState(false);
              this.serverRestartRequired(true);
              this.successModal(this.$t("userAddress.deleteSuccess"));
            },
            (error) => {
              console.log("httperror  => ", error);
              this.setLoadingState(false);
    ...
    editUserAddress(item) {
      this.setUserAddress(item.item);
      this.$router.push("/user-address/update/" + item.item.id);
    },
  },
};
</script>